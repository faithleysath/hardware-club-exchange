...
- 已完成成员登录、闲置发布与审核、搜索筛选、我的发布和管理员审核主链路。
- 草稿保存、重新送审和多图编辑已并入当前版本。
...
- `reserved` / `已预订` 状态现在由预约流中的“接受预约”自动驱动，不再作为“我的发布”页的手动快捷状态。
- 卖家释放候补或关闭成交，也通过“我的预约”页中的预约处理动作推进。
...
- 核心交付项已落地。
- 买家预约申请、卖家接受 / 拒绝 / 取消 / 关闭成交、我的预约页、收藏能力、举报能力、举报处理页均已实现。
- 审计日志查询基础界面已落地。
